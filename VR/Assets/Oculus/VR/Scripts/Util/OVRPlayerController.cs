		...
		if (EnableRotation)
		{
			...
			if (SnapRotation)
			{
                if (OVRInput.Get(OVRInput.Button.PrimaryThumbstickLeft) ||
					(RotationEitherThumbstick && OVRInput.Get(OVRInput.Button.SecondaryThumbstickLeft)))
				{
					if (ReadyToSnapTurn)
					{
						euler.y -= Time.deltaTime * 10;
						//ReadyToSnapTurn = false;
					}
				}
				else if (OVRInput.Get(OVRInput.Button.PrimaryThumbstickRight) ||
					(RotationEitherThumbstick && OVRInput.Get(OVRInput.Button.SecondaryThumbstickRight)))
				{
					if (ReadyToSnapTurn)
					{
						euler.y += Time.deltaTime * 10;
			...
			else
			{
				...
				{
					Vector2 altSecondaryAxis = OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick);
